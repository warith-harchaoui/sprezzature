...
- `sprezzature` is narrow on purpose: ten skills, one frontend stack (vanilla JS + Tailwind), local-first execution via [`best-engine-ai-helper`](https://github.com/warith-harchaoui/best-engine-ai-helper). Use LangChain when your problem is retrieval at scale; use `sprezzature` when the problem is frontend quality at the human-document level.
