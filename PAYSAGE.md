...
- `sprezzature` est délibérément étroit : dix *skills*, une seule pile frontend (JS pur + Tailwind), exécution locale via [`best-engine-ai-helper`](https://github.com/warith-harchaoui/best-engine-ai-helper). Choisissez LangChain quand votre problème est la récupération à grande échelle ; choisissez `sprezzature` quand le problème est la qualité frontend au niveau du document humain.
