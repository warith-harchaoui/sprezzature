...
| **sprezzature-maps** | `make-map choropleth` (rows keyed by territory → shaded real basemap, Equal Earth, OKLCH ramp, relief), `make-map situation_map` (one YAML config → layered areas-of-control plate, auto-centred Lambert conformal conic, bathymetry, dual-unit scale bar) | _(none here — the rendered SVG is a figure, so `sprezzature-figures`' `audit_figure.py` and its colour-vision levels apply unchanged. One auditor, two producers.)_ |
...
| A data shape (CSV, JSON, a few rows) | "Chart this" / "Dashboard for X" | `sprezzature-ui` | Inline SVG + `<figure>` wrapper. House style, palette from `color-psychology.md`, polarity-tagged axes, `role="img"`. |
