...
# On réutilise les fondations partagées du dépôt.
_SCRIPTS = Path(__file__).resolve().parent.parent / "sprezzature-figures" / "scripts"
...
# --------------------------------------------------------------------------- #
#: Style commun injecté dans chaque panneau : les cibles interactives portent la
#: classe ``hit`` et révèlent leur info-bulle ``.tip`` au survol / focus. Aucune
#: animation — juste un révélateur d'information au survol (et un focus clavier).
BASE_STYLE = (
...
def svg_open(w: float, h: float, label: str, style: str = "") -> str:
    ...
    style : str
        CSS spécifique au panneau, ajouté après :data:`BASE_STYLE`.
...
__all__ = [
    ...
    "MONO", "NBSP", "f1", "Box", "fr_num", "fr_pct", "nice_ticks", "year_ticks",
    "smooth_path", "polyline_path", "svg_open", "tip_group", "BASE_STYLE",
