...
|---|---|---|
| `shap` | `summary_bar.svg`, `summary_beeswarm.svg`, one `dependence_<feature>.svg` per top-N feature (`--top-n`, default 20 for the bar/beeswarm, default 5 for the dependence loop), `waterfall_row_<i>.svg`, `shap_values.parquet` (best-effort; skipped with a warning if `pyarrow` is not installed) | Every plot is hand-authored SVG, no matplotlib, no Vega. The bar and waterfall reuse the catalogue's `make_bar.py` / `make_waterfall.py` generators; the beeswarm and dependence scatter are bespoke (SHAP's per-feature swarm rows and arbitrary x/y roles don't fit either catalogue generator's fixed data contract). The waterfall row defaults to whichever row has the largest absolute prediction; override with `--waterfall-row`. |
| `shapash` | `report.html`, `summary_bar.svg`, `summary_beeswarm.svg`, `waterfall_row_<i>.svg`, `smart_explainer.pkl` (best-effort) | `SmartExplainer.compile()` is fed this run's own SHAP contributions directly (rather than recomputing or introspecting Shapash's internal attribute names) and still runs Shapash's own consistency checks; `report.html` is a static page this module assembles from the same SVG plots the `shap` engine produces, not Shapash's own plotly-backed `generate_report()` dashboard, which is never called. |
| `timeshap` | `timeshap_<name>.csv` per report section, best-effort `timeshap_<name>.svg` for any section shaped as a label + `Shapley Value` column, `timeshap_report.json` (the pruning/event/feature/cell parameter dicts actually used) | Requires `--sequence-cols` (comma-separated). Raises a clear `SystemExit` naming the missing package if TimeSHAP is not installed, rather than a bare `ImportError`. `timeshap.plot`'s matplotlib-backed plotting functions are never called; `local_report`'s own returned dataframes are re-plotted by hand where their shape fits, else left as CSV only. |
| `lime` | `lime_row_<i>.html` for each of `--n-explain` rows (default 10) | Uses `predict_proba` when the model has one, otherwise `predict`. Already matplotlib-free before this pass: LIME writes its own standalone interactive HTML per row. |
