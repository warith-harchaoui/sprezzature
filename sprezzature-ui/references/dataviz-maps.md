...
- For interactive map UIs at scale, hand-author the projection as inline SVG paths, or reach for a dedicated mapping runtime (MapLibre GL, Leaflet) when the map has to pan and zoom over live tiles. Keep the house chart style (same Roboto type, same brand palette, same 10 px rounded corners on chrome); see `charts-svg.md`.
